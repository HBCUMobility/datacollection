...
# This script will fetch a URI-Ms in TimeMaps and generate a WARC from each URI-M.
...
def create_warc_from_timemap(tm_path):
    ...
    while ll := f.readline().strip():
        if ll[0] == '!': continue  # Exclude CDXJ metadata lines
...
for root, dirs, files in os.walk(os.path.abspath('./timemaps/')):
    for file in files:
        if file[0] == '.': continue  # Exclude dotfiles
